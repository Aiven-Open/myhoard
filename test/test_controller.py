...
def test_periodic_backups_are_paused(time_machine, master_controller) -> None:
    # pylint: disable=protected-access
    time_machine.move_to("2023-01-02T18:00:00")
    ...
    expected_normalized_time = datetime.datetime(2023, 1, 11, 3, tzinfo=datetime.timezone.utc)
...
def test_get_latest_processed_binlog_index(streams: List[MagicMock], expected: Optional[int]) -> None:
    controller = Controller.__new__(Controller)
    controller.backup_streams = cast(List[BackupStream], streams)
    assert controller.get_latest_processed_binlog_index() == expected
