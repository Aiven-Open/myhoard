...
How XtraBackup should lock DDL while taking a basebackup, either ``ON`` or
``REDUCED``. When not set (the default) ``--lock-ddl`` is not passed at all and
XtraBackup's own default applies, which is ``ON`` at the time of writing. An
explicit value is always passed through, so it does not depend on XtraBackup's
default. For a value MyHoard does not recognise a warning is logged and
``--lock-ddl`` is not passed, so XtraBackup's default applies.
...
With ``REDUCED`` the InnoDB data is copied without the lock, DDL is tracked
through the redo log, and a short lock is taken at the end of the backup only to
reconcile the tables that DDL touched. That keeps the lock window roughly
constant regardless of how much data there is. ``REDUCED`` requires Percona
XtraBackup 8.4 or newer (and therefore MySQL 8.4 or newer). On older versions a
warning is logged and ``--lock-ddl`` is not passed, so XtraBackup's default
applies. It works for both full and incremental backups but cannot be combined
