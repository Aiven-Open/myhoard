...
class Controller(threading.Thread):
    ...
    def get_latest_uploaded_binlog_index(self) -> Optional[int]:
        """Highest local binlog index that every stream in the regular binlog phase has uploaded.

        Only completed streams are restore candidates, so a stream still catching up after a new
        basebackup does not count until it completes; is_log_backed_up is stricter and waits for it.
        None when no stream is in the regular binlog phase or one of them has uploaded nothing yet.
        """
        indexes = [
