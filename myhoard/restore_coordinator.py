...
class RestoreCoordinator(threading.Thread):
    ...
    def get_backup_info(self) -> None:
        ...
        required_backups = []
        if basebackup_info["incremental"]:
            # We need to load all backups infos until the latest full one
            for stream_id in basebackup_info["required_streams"]:
            ...
            try:
                if required_backups:
                    self.log.info(
                        ...
                        restore_basebackup_info["required_streams"],
                    )
                    for idx, required_backup in enumerate(required_backups, 1):
                        ...
                        self.log.info(
                            "Preparing required backup (incremental: %r): %r (%r/%r)",
                            required_basebackup_info["incremental"],
                            stream_id,
                            idx,
                            len(required_backups),
                        )
                        assert required_basebackup_info["incremental"] == (idx != 1)
